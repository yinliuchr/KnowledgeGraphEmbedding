...
class ConvModel(nn.Module):
    ...
    def forward(self, sample):              # sample is  size: (batch_size ,3)
        ...
        # x = self.bn2(x)
        # x = F.relu(x)  # 1 * 200
        # x = torch.mm(x, e2_embedded.weight.transpose(1, 0))  # 1 * 200  @ (200 * # ent)  => 1 * # ent
                ...
                for test_dataset in test_dataset_list:
                    for positive_sample, negative_sample, filter_bias, mode in test_dataset:
                        ...
                        if mode == 'head-batch':
                            ...
                            for i in range(bs):
                                ori_sam = positive_sample[i].repeat(ns, 1)  # 14951 * 3         positive_sample[i] = [32,971, 18]
                                ori_sam[:, 2] = negative_sample[i]
                                if args.cuda:
                                    ori_sam = ori_sam.cuda()
                                # print('\n**************************\nori_sam_dim: ', ori_sam.size(),
